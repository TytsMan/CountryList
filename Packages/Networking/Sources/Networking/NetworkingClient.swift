...
public final class NetworkingClient {
    ...
    private func decode<T: Decodable>(data: Data, responseType: T.Type) -> Result<T, NetworkingError> {
        do {
            ...
            #if DEBUG
            if let decondingError = error as? DecodingError {
                let debugDescription = switch decondingError {
                ...
                print("Deconding error: \(debugDescription)")
            }
            #endif
            return .failure(.internalStatus(status: .decodingFailed, message: error.localizedDescription))
        }
    }
}
